...
private fun ElegirVehiculo(
   ...
   val type by remember { mutableStateOf(vehiculo.type) }
      ...
      Row {
         Column {
            ...
            when (listVehiculo.indexOf(vehiculoElegido1)) {
               0 -> FormularioCoche(obtenerVehiculo, gps, type)
               1 -> FormularioMoto(obtenerVehiculo, gps, type)
...
@Composable
fun FormularioMoto(obtenerVehiculo: (Vehiculo) -> Unit, gps: Boolean, actualType: String) {
   var type by remember { mutableStateOf(actualType) }

   TipoMoto(actualType = when (actualType) {
      "250" -> R.string._250
      "125" -> R.string._125
      "50" -> R.string._50
      else -> R.string.moto
   }, onCambiarTipo = { type = it })
   HorizontalDivider()
...
@Composable
fun TipoMoto(@StringRes actualType: Int, onCambiarTipo: (String) -> Unit) {
   ...
   onCambiarTipo(
      when (tiposMotos.indexOf(tipoMoto)) {
...
@Composable
fun FormularioCoche(obtenerVehiculo: (Vehiculo) -> Unit, gps: Boolean, actualType: String) {
   var type: String by remember { mutableStateOf(actualType) }


   TipoCoche(actualType = when (actualType) {
      "die" -> R.string.diesel
      "gas" -> R.string.gasolina
      "ele" -> R.string.electrico
      else -> R.string.coche
   }, onCambiarTipo = { type = it })
...
@Composable
fun TipoCoche(onCambiarTipo: (String) -> Unit, @StringRes actualType: Int) {
   val tiposCoche = listOf(R.string.diesel, R.string.gasolina, R.string.electrico)
   val (tipoCoche, onEleccion) = remember { mutableIntStateOf(actualType) }
